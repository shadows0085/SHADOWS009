...
export class UploadController {
  ...
  static async uploadSingle(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const category = (req.body.category === 'thumbnails' ? 'thumbnails' : 'videos') as 'videos' | 'thumbnails';

      const result = await UploadService.processUploadedBuffer(
        req.file.buffer,
   ...
   */
  static async initiateChunked(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { fileName, mimeType, fileSize } = req.body;
      const session = await UploadService.initiateUploadSession(
        req.admin!.adminId,
        fileName,
        mimeType,
        fileSize
   ...
   */
  static async appendChunk(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { sessionId } = req.params;
      const chunkIndex = parseInt(req.query.chunkIndex as string, 10) || 0;
      ...
      const progress = await UploadService.appendChunk(sessionId, req.admin!.adminId, req.file.buffer, chunkIndex);
   ...
   */
  static async finalizeChunked(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { sessionId } = req.params;
      const assembled = await UploadService.finalizeUploadSession(sessionId, req.admin!.adminId);
  ...
  static async getStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { sessionId } = req.params;
      const status = await UploadService.getSessionStatus(sessionId, req.admin!.adminId);
      res.status(200).json({
        success: true,
        data: status
      });
  ...
  static async cancel(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { sessionId } = req.params;
      const result = await UploadService.cancelSession(sessionId, req.admin!.adminId);
