...
export class VideoController {
  ...
  static async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const video = await VideoService.createVideo(req.admin!, req.body, ip, ua);
      res.status(201).json({
        success: true,
        data: video
      });
  ...
  static async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const updated = await VideoService.updateVideo(id, req.admin!, req.body, ip, ua);
      res.status(200).json({
        success: true,
        data: updated
      });
  ...
  static async updateStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const updated = await VideoService.updateVideo(id, req.admin!, { status }, ip, ua);
      res.status(200).json({
        success: true,
        data: updated
      });
  ...
  static async delete(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await VideoService.deleteVideo(id, req.admin!, ip, ua);
      res.status(200).json({
        success: true,
        data: result
      });
   ...
   */
  static async getStreamTicket(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      const ip = req.ip || req.socket.remoteAddress || '127.0.0.1';
      const ua = req.headers['user-agent'] || 'unknown';

      const ticket = await VideoService.generateStreamTicket(id, req.admin!, ip, ua);
      res.status(200).json({
        success: true,
        data: ticket
      });
   ...
   */
  static async streamMedia(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { ticket } = req.params;
      const ip = req.ip || req.socket.remoteAddress || '127.0.0.1';
      const ua = req.headers['user-agent'] || 'unknown';

      const { filePath, fileSize, mimeType } = await VideoService.getVideoStream(ticket, ip, ua);
  ...
  static async getMetrics(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const metrics = await VideoService.getDashboardMetrics();
      res.status(200).json({
        success: true,
        data: metrics
      });
