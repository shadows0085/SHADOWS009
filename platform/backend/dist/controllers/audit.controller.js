...
exports.AdminController = exports.AuditController = void 0;
...
class AuditController {
    static async list(req, res, next) {
        try {
            ...
            const result = await audit_service_1.AuditService.listAuditLogs({
...
class AdminController {
    ...
    static async create(req, res, next) {
        try {
            ...
            // ═══════════════════════════════════════════════════════════════════
            if (role === 'SUPER_ADMIN') {
                const existingSuperCount = await admin_repository_1.AdminRepository.countActiveSuperAdmins();
            ...
            const existing = await admin_repository_1.AdminRepository.findByEmail(email);
            if (existing) {
                res.status(409).json({
                    success: false,
    ...
    static async update(req, res, next) {
        try {
            ...
            if (isTargetActiveSuperAdmin && (willDemote || willDeactivate)) {
                const activeSuperCount = await admin_repository_1.AdminRepository.countActiveSuperAdmins();
            ...
            if (password && password.trim().length >= 6) {
                updateData.passwordHash = await crypto_1.CryptoUtils.hashPassword(password.trim());
            }
            const updated = await admin_repository_1.AdminRepository.updateWithSuperAdminGuard(id, updateData);
            res.status(200).json({
                success: true,
                data: {
                    id: updated.id,
    ...
    static async toggleStatus(req, res, next) {
        try {
            ...
            const target = await admin_repository_1.AdminRepository.findById(id);
            if (!target) {
                res.status(404).json({
                    success: false,
                    error: { code: 'ADMIN_NOT_FOUND', message: 'Administrator account not found.' }
            ...
            const updated = await admin_repository_1.AdminRepository.updateWithSuperAdminGuard(id, { isActive: Boolean(isActive) });
            res.status(200).json({
                success: true,
                data: {
                    id: updated.id,
    ...
    static async delete(req, res, next) {
        try {
            ...
            await admin_repository_1.AdminRepository.deleteWithSuperAdminGuard(id);
            res.status(200).json({
                success: true,
